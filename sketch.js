...
function preload() {
  ...
  // Many example projects supply multiple numbered punch files (punch_1..punch_9).
  // This project only has a single `Punch.mp3`, so load it multiple times
  // so the rest of the code that picks a random punch still works.
  let singlePunch = loadSound("assets/sounds/Punch.mp3");
  for (let i = 0; i < 9; i++) {
    punchSounds.push(singlePunch);
  }
  winSound = loadSound("assets/sounds/win.mp3");
  bgMusic = loadSound("assets/sounds/background.mp3");
  WalkSound = loadSound("assets/sounds/Walking.mp3");
...
function drawStartScreen() {
  // Background box behind title and subtitle
  let boxWidth = 520;
  let boxHeight = 140;
  let boxX = width / 2;
  ...
  textSize(52);
  noStroke();
  text("Magic Fighters", width / 2, height / 2 - 60);
...
function updateAndDrawFighters() {
  ...
  // Play walking sound while either fighter is moving horizontally.
  // Use a small threshold to avoid playing for tiny friction velocities.
  let movingThreshold = 0.5;
  let anyMoving =
    abs(fighter1.vx) > movingThreshold || abs(fighter2.vx) > movingThreshold;
  if (WalkSound) {
    if (anyMoving) {
